...
function PlannerPage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 pb-12">
        <section className="overflow-hidden rounded-[32px] border border-line bg-panel text-ink shadow-2xl">
          <div className="grid gap-8 px-6 py-8 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:px-10 lg:py-10">
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                <h1 className="max-w-3xl text-3xl font-semibold tracking-tight text-ink sm:text-4xl">Planner</h1>
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                <label className="block space-y-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-line bg-canvas px-4 py-3 text-sm text-ink outline-none transition placeholder:text-muted focus:ring-2 focus:ring-accent/50"
                  />
                </label>
                <button
                  type="submit"
                  disabled={status === "loading"}
                  className="inline-flex w-full items-center justify-center rounded-2xl border border-line bg-accent/20 px-5 py-3 text-sm font-bold text-accent transition hover:bg-accent/40 disabled:cursor-not-allowed disabled:opacity-70 active:scale-[0.98]"
                >
        ...
        <section className="grid gap-6 lg:grid-cols-[0.8fr_2fr]">
          <aside className="rounded-[28px] border border-line bg-panel p-6 shadow-panel h-fit sticky top-8">
            <div className="space-y-6">
              <div>
                ...
                <h2 className="mt-2 text-lg font-bold text-ink">Current Plan</h2>
              </div>
              <dl className="space-y-4">
                {[
                  ...
                  <div key={item.label} className="rounded-2xl border border-line bg-canvas/50 p-4 transition-all hover:bg-line/30">
                    <dt className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted">{item.label}</dt>
                    <dd className={`mt-1 break-words font-semibold text-ink ${item.large ? 'text-3xl tracking-tight' : 'text-sm'}`}>
                      {item.value}
                    </dd>
                  </div>
                ))}
          ...
          <section className="space-y-6">
            <div className="rounded-[28px] border border-line bg-panel p-6 shadow-panel">
              ...
              {error ? <p className="mt-4 rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-sm font-bold text-red-400">{error}</p> : null}
            </div>
            ...
            {result ? (
              <>
                ...
                <div className="mt-8 overflow-hidden rounded-[32px] border border-line bg-panel p-8 shadow-2xl text-ink">
                  <div className="flex flex-col items-center justify-between gap-6 sm:flex-row">
                    <div className="space-y-1 text-center sm:text-left">
                      <h3 className="text-xl font-bold">Consolidated Research Report</h3>
                      <p className="text-sm text-muted">Download the full plan as a structured document.</p>
                    </div>
              ...
              <div className="rounded-[32px] border border-dashed border-line bg-panel p-16 text-center shadow-sm">
                ...
                <h3 className="text-xl font-bold text-ink">Ready to Plan</h3>
