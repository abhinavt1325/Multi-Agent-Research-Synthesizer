...
def run_contradiction_detector(claim_a: str, claim_b: str) -> dict[str, Any]:
    ...
    if settings.gemini_api_key:
        try:
            result = _call_gemini(normalized_claim_a, normalized_claim_b, settings.gemini_api_key)
            return {
                "provider": "gemini",
                **result,
            }
    ...
    if not settings.groq_api_key and not settings.gemini_api_key:
        raise ContradictionDetectorServiceError(
            "Contradiction Detector is unavailable. Set GROQ_API_KEY or GEMINI_API_KEY.",
            status_code=503,
        )
